...
class EditVideo extends Component {
  ...
  handleSubmit = async () => {
    if (this.isValid()) {
      ...
      const response = await API.UpdateVideoAPI(this.state.videoId, formData);
      if (response.data.status == 200) {
        createNotification("success","Video Updated");
        this.setState({ loader: false },()=>{
          this.props.history.push('/trainer/list-video')
        });
      } else {
...
export default connect(mapStateToProps, mapActionsToProps)(EditVideo);
