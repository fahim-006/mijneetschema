...
class FeaturedProducts extends Component {
    ...
    handleAddToCart = (e) => {
        ...
        if (isExist === parseInt(-1)) {
            ...
            e = Object.assign({ 'Aantal': 1 }, e)
            stateCart.push(e);
            this.setState({
                cart_Array: stateCart
            }, () => {
                createNotification('success', `Product ${e.name} is successvol toegevoegd aan jouw winkelwagen.`);
                localStorage.setItem('items_in_cart', JSON.stringify(this.state.cart_Array));
            });
        } else {
            createNotification('info', 'Dit item zit al in jouw winkelwagen.');
        }
    }
    ...
    handleAddWish = (e) => {
        if(localStorage.getItem("user_role") && localStorage.getItem("user_role") !== "undefined" && localStorage.getItem("user_role") ==='3')
        {
            ...
            createNotification('info', 'Om toe te voegen aan jouw wensen lijst moet je eerst inloggen!!');
        }
