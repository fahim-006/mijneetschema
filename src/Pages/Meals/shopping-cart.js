...
class CartPage extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            totalPrice: '',
            cartProducts: [],
            coupon: '',    // For now, we consider 10% discount for coupan
    ...
    handleQuantityChange = (event, index) => {
        ...
        currentItem.quantity = event.target.value === 'NaN' ? 0 : parseInt(event.target.value);
        // currentItem.quantity = parseInt(event.target.value);
        stateCart[index] = currentItem;
        this.handleCartUpdates(stateCart, '');
        // this.setState({
        //     cartProducts: stateCart
        // }, () => {
        //     localStorage.setItem('items_in_cart', JSON.stringify(this.state.cartProducts));
        // });
    }
    ...
    handleRemoveCartItem = (index) => {
        ...
        if (newCart.length !== stateCart) {
            ...
            // }, () => {
            //     localStorage.setItem('items_in_cart', JSON.stringify(this.state.cartProducts));
            //     createNotification('success', 'Item removed from cart successfully')
            // });
        }
    }
    ...
    calculatePriceAndItems = () => {
        const { cartProducts } = this.state;
        let t_items = cartProducts.reduce((count, item) => parseInt(count + item.quantity), 0);
        let t_price = cartProducts.reduce((price, item) => parseFloat(price + (item.quantity * item.price)), 0.00);
    ...
    isAddressValid = () => {
        const { name, contact_no, pincode, city, address, landmark, state, userId, email, password } = this.state;
        let error = {};
        ...
        if (userId === '' && email === '') {
            formIsValid = false;
            error['email'] = "*Enter email address (If you are already registered please login first)."
        }
    ...
    handleDeliverProcess = () => {
        ...
        if (this.isAddressValid()) {
            ...
            if (localStorage.getItem('items_in_cart') !== undefined) {
                ...
                t_Items = items.map(itm => itm._id);
            }
            const {
                userId, name, email, contact_no, address, city,
                pincode, landmark, state, alternate_phone, product_description,
                totalItems, totalPrice, password, amount_without_tax, amount_with_tax
            } = this.state;
            ...
            let requestBody = {
                ...
                address: address,
                amount_without_tax: totalPrice,             // amount_without_tax,
                amount_with_tax: totalPrice,                // amount_with_tax,
                total: totalPrice,   
    ...
    componentDidUpdate(prevProps, prevState) {
        if (
            prevProps.prod_payment_url !== this.props.prod_payment_url
        ) {
            window.location = `${this.props.prod_payment_url}`;
        }

        if(this.props.apply_success && prevProps.apply_success !== this.props.apply_success){
            this.handleCouponDiscount();
            createNotification('success', 'Coupon succesvol toegevoegd');
        }
        if(this.props.apply_error && prevProps.apply_err_msg !== this.props.apply_err_msg){
    ...
    handleCouponDiscount = () => {
        ...
        let price_with_coupan = ttl_price - (parseFloat(ttl_price*discount).toFixed(2));
    ...
    applyCoupan = () => {
        ...
        if (coupon) this.props.applyCoupan({coupon: coupon}); 
    }
    ...
    render() {
        const { cartTab, cartProducts, totalItems, totalPrice, coupon, amount_with_tax, amount_without_tax, isCuopanApplied } = this.state;
        return (
            <ProductsLayout>
                        ...
                        <div className="col-12">
                            <div className="steps_result">
                                <div className="banner_txt d-flex">
                                    <div className="banr_headng">Jouw <br />winkelwagen</div>
                            ...
                            <div className="row all-products-titles place-order-titles">
                                <div className="col-md-12">
                                    <ul className="nav nav-tabs">
                                        <li>
                                            <button className="active" onClick={e => this.handleCartTab(1)}>
                                        ...
                                        {/* <li><a className="active" href="shopping-cart.html"><i className="fa fa-check-circle"></i> Shopping Cart</a></li>
                                        <li><a href="checkout.html"><i className="fa fa-check-circle"></i> Checkout</a></li> */}
                                    </ul>
                                </div>
                            </div>
                                                        ...
                                                        {cartTab === parseInt(1) ?
                                                            <div className="block cart_table_wrapper">
                                                                <div className="heading-cart-row">
                                                                    <div className="heading-cart-col cart-image">product</div>
                                                                    <div className="heading-cart-col cart-name">naam</div>
                                                                    <div className="heading-cart-col cart-price">prijs</div>
                                                                    <div className="heading-cart-col cart-qut">aantal</div>
                                                                    <div className="heading-cart-col cart-select">status</div>
                                                                </div>
                                                    ...
                                                    <div className="diet_item prod-con">
                                                        <div className="block">
                                                            <div className="heading-cart-row">
                                                                ...
                                                                <div className="heading-cart-col price-details-text">Prijs ({totalItems} items)
                                                                    <span className="price-cart">€ {totalPrice}</span>
                                                                </div>
                                                                <div className="heading-cart-col price-details-text">Delivery Fee
                                                                    <span className="price-cart">€ 0</span>
                                                                </div>
                                                                <div className="heading-cart-col price-details-text">BTW (21%)
                                                                    <span className="price-cart">€ {parseFloat(totalPrice*(21/100)).toFixed(2)}</span>
                                                                </div>
                                                                {isCuopanApplied?
                                                                    <div className="heading-cart-col price-details-text">Coupon korting
                                                                        <span className="price-cart">
                                                                            - € {
                                                                                parseFloat(totalPrice*(parseInt(this.props.discount)/100)).toFixed(2)
                                                                            }
                                                                        </span>
                                                                    </div>
                                                                    :
                                                                ...
                                                                <div className="heading-cart-col final-price">Totaal
                                                                ...
                                                                <div className="heading-cart-col coupon-code-op">
                                                                    ...
                                                                    {coupon && coupon.length >= 2 ? 
                                                                        <div className="click_btn">
                                                                            ...
                                                                            >
                                                                                {isCuopanApplied ? 'Probeer een andere' : 'Coupon code'}
                                                                            </button>
                                                                ...
                                                                {/*
                                                                    <div className="heading-cart-col delivery-select">
                                                                        <i className="fa fa-map-marker"></i> Deliver to
                                                                        <div className="heading-cart-col delivery-select-inner">
                                                                            <select name="cars" id="cars">
                                                                                <option value="Adress1">Adress 1</option>
                                                                                <option value="Adress2">Adress 2</option>
                                                                            </select>
                                                                        </div>
                                                                    </div> 
                                                                */}
